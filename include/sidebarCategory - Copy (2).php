...
<div id="filter" >
    ...
    <?php /*<ul id="putCatHere"> <strong> ALL CATEGORIES </strong>
	...
	if(isset($_GET['gender']) && $_GET['gender'] != ''){
		if($_GET['gender'] != 'all'){
			$genderForRedirect = $_GET['gender'].'s';
		}else{
			$genderForRedirect = 'all';
		}
	}
	
	if(isset($_GET['subSubCat']) && $_GET['subSubCat'] != ''){
		$catData1 = getCategoryFromSlug(array('category_id'), $_GET['subSubCat'], $con);
		$catForAjax = $catData1->category_id;
		
		// start building sidebar
		$catData = getCategoryFromSlug(array('category_id', 'category_name', 'slug'), $_GET['sub'], $con);
		$gen = $genderForRedirect; $catId = $catData->category_id;
		...
		<strong>
			...
			<br/>>
			ALL <?php echo $catData->category_name;?>
		</strong>
		<?php while($cat_row = mysql_fetch_object($cat_rs)){ ?>
		<li>
    ...
    elseif(isset($_GET['sub']) && $_GET['sub'] != ''){
		$catData = getCategoryFromSlug(array('category_id', 'category_name', 'slug'), $_GET['sub'], $con);
		$catForAjax = $catData->category_id;
	...
	elseif(isset($_GET['category']) && $_GET['category'] != ''){
		$catData = getCategoryFromSlug(array('category_id', 'category_name', 'slug'), $_GET['category'], $con);
		$catForAjax = $catData->category_id;
		
		// start building sidebar
		$catId = $catData->category_id;
		$catName = $catData->category_name; $gen = $genderForRedirect;
		// main cat
		$slugRs = exec_query("SELECT slug FROM tbl_category WHERE category_id = '$catId'", $con);
		$slug = mysql_fetch_object($slugRs)->slug;
		// sub cat
		...
		<strong>
			...
			<a href="<?php echo siteUrl; ?>all/">ALL CATEGORIES</a><br/>>
			<a href="javascript:void(0);" onclick="getMaleFemaleOnCategoryPage(<?php echo $catId.", '".$catName."'"; ?>);">ALL <?php echo $catData->category_name;?></a><br/>   
		</strong>
		<?php
		if($gen != 'all'){
			if($gen == 'men'){ $gen = 'mens'; } else{ $gen = 'womens'; }
		}
		while($cat_row = mysql_fetch_object($cat_rs)){ ?>
		<li>
		...
		?> <strong> ALL CATEGORIES </strong> <?php
		// if nothing is set
    ...
    <ul id="brand"> <strong style="float:left;"> BRANDS </strong> <a href="javascript:void(0);" style="float:right;" onclick="clearBrandCondition();"><strong>X</strong> Reset</a>
    	<form action="<?php echo $actual_link = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"; ?>" method="post" id="bFormB">
		...
		$brandIdForChk = '';
		if(isset($_GET['searchByBrand']) && $_GET['searchByBrand'] != ''){ $brandIdForChk = $_GET['searchByBrand']; }
		
		$brandArrr = array();
		if(isset($_POST['brandFilter']) && !empty($_POST['brandFilter'])){
			$brandArrr = $_POST['brandFilter'];
		}
		
		while($br_row = mysql_fetch_object($br_rs)){
			if($br_row->totalProduct != 0){	$countBPro = $br_row->totalProduct; ?>
            <li>
            	<?php /*<input name="brand" type="checkbox" value="<?php echo $br_row->brand_id; ?>" onchange="setMultiChkValuesBrand('brand');" />*/ ?>
                <input name="brandFilter[]" type="checkbox" value="<?php echo $br_row->brand_id; ?>" onchange="bFormB.submit();" <?php echo (in_array($br_row->brand_id, $brandArrr) || $brandIdForChk == $br_row->brand_id)?'checked="checked"':''; ?> />
