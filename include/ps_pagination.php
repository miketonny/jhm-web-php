...
class PS_Pagination {
	...
	function renderFirst($tag = 'Newer') {
		if ($this->total_rows == 0)
			return FALSE;
		
		if(isset($_SESSION['spanish'])){ $tag = 'Anteriores'; }
	 ...
	 * @return string
	 */
	function renderLast($tag = 'Older') {
		if ($this->total_rows == 0)
			return FALSE;
		
		if(isset($_SESSION['spanish'])){ $tag = 'Posteriores'; }
