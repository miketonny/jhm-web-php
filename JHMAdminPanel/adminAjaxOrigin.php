...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'getSubCategory'){
	...
	if(isset($_GET['data1'])){
		...
		$rs = mysql_query("SELECT category_id, category_name FROM tbl_category WHERE parent_id = 0 AND $condi ORDER BY category_name", $con);
...
elseif(isset($_REQUEST['action']) && $_REQUEST['action'] == 'chkProductFields'){
	...
	if(isset($_GET['type']) && $_GET['type'] != ''){
		$pid = $_GET['pid'];
		$rs = mysql_query("SELECT product_id FROM tbl_product WHERE $column = '$value' AND $column != '' AND product_id != '$pid'", $con);
	}
	else{
		$rs = mysql_query("SELECT product_id FROM tbl_product WHERE $column = '$value' AND $column != ''", $con);
	}
...
elseif(isset($_REQUEST['action']) && $_REQUEST['action'] == 'getSku'){
	...
	if(mysql_num_rows($skuRs)){
		$i = 1;
		while($row = mysql_fetch_object($skuRs)){
